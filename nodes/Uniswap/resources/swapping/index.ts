...
export const swappingDescription: INodeProperties[] = [
		...
		},
		{
			...
			"type": "json",
			"default": "[\n  {}\n]",
			"description": "Optional integrator fee configuration. When provided, the specified fees are applied to the swap instead of the default partner fee service. Up to 4 fee entries are supported on both UniswapX and classic (Universal Router) routes. Multi-entry fees on a Universal Router route require router version 2.1.1: the API resolves to 2.1.1 automatically on chains where it is deployed, and an explicit `x-universal-router-version` pin below 2.1.1 (or a chain without a 2.1.1 deployment) is rejected with a 400. Multi-entry fees are exact-input only; an exact-output request with more than one entry is rejected with a 400. Rejections carry a machine-readable `errorCode`: `IntegratorFeesTooManyEntriesError` (more than 4 entries), `IntegratorFeeEntryExceedsMaxError` (one entry above 500 bps), `IntegratorFeesTotalExceedsMaxError` (entries summing above 500 bps), and `InvalidIntegratorFeesError` for any other malformed shape (duplicate recipient, non-positive or over-precise bips, exact-output with several entries, router version below 2.1.1). The encoded calldata is checked against the advertised fee legs before it is returned, and a mismatch withholds the calldata. Each recipient may appear at most once, and the `bips` SUMMED across all entries must not exceed 500 (5%) — the total is capped at 500 regardless of whether 1, 2, 3, or 4 fees are passed (a request whose fees sum higher is rejected with a 400: 'integratorFees total bips must be less than or equal to 500, but was <total>'). The caps are enforced server-side on every transport and re-checked at /swap. An empty array is accepted and means no integrator fees, the same as omitting the field.",
			"routing": {
				"send": {
